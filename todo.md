...
- [x] 在 `games` 表保存名称、说明、运行状态、接入状态、客户端可见性、顺序和 revision。
- [x] 新增独立的 `can_manage_games` 全局权限，不复用逐游戏账号权限。
- [x] 提供管理员游戏项目列表、新增和编辑 API，不提供删除或修改 `gameId`。
- [x] 新项目固定为 `draft + maintenance` 且不下发；技术配置通过启动校验后转为
  `configured`。
- [x] 使用 revision 乐观锁并保持 `disabled` 终态。
- [x] 提供 Public `GET /v1/games`，仅下发 configured、显式可见且未停用的游戏。
- [x] 在管理员网页增加游戏项目列表、新增/编辑表单、客户端下发开关和展示顺序。
- [x] 游戏项目变更写入独立审计，不让微信、目录文件路径或 Secret 进入网页。

## 12. 每个游戏的区服管理

- [x] 新增按 `(game_id, server_id)` 隔离的区服表和目录设置表。
- [x] 首次接入从 `directoryPath` 导入区服，之后以数据库为唯一真源且启动不覆盖。
- [x] 提供管理员区服列表、新增和编辑 API；`serverId` 创建后不可修改或删除。
- [x] 使用 `revision` 乐观锁，重复 `serverId` 或过期版本返回
  `GAME_SERVER_CONFLICT`。
- [x] 区分“未开放”和“维护中”：未开放不下发，已开放的维护区服继续下发但拒绝登录。
- [x] Public 区服列表只返回开放区服并按顺序排列，不改变 `AreaServer` 响应结构。
- [x] 在游戏项目页增加区服管理对话框、字段校验、加载/空/错误和冲突状态。
- [x] 区服新增与编辑写入游戏管理审计，并补齐契约、领域、集成和浏览器测试。
